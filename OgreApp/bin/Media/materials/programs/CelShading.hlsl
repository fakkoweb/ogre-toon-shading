...
VS_OUTPUT CelShadingVS( VS_INPUT input )
{
	...
	output.edgeFactor			= max( dot( N, E ), 0 );
...
uniform sampler1D edgeGradient		: register( s2 );
...
float4 CelShadingPS( VS_OUTPUT input ) : SV_Target
{
	...
	float edgeFactor		= tex1D(edgeGradient, input.edgeFactor).x;

	float4 color			= edgeFactor * 
								((diffusiveComponent * diffusiveFactor) + 
